...
public class AStar
{
	...
	/*
	public static void init(TerrainTile[][] map, MapLocation start, MapLocation end)
	{
		mapWidth = map.length;
		mapHeight = map[0].length;
		realStart = start;
		realEnd = end;
		AStar.start = new MapLocation(realEnd.x, realEnd.y);
		AStar.end = new MapLocation(realStart.x, realStart.y);
		AStar.map = map;

		MapLocationComparator comparator = new MapLocationComparator();
		...
		open.add(AStar.start);
		g[AStar.start.x][AStar.start.y] = 0;
		h[AStar.start.x][AStar.start.y] = heuristic(AStar.start.x, AStar.start.y, AStar.end.x, AStar.end.y);
	}
	*/

	/*
	public static ArrayList<MapLocation> find(int turns)
	{
		int startTurn = Clock.getRoundNum();
		while (!open.isEmpty() && Clock.getRoundNum() - startTurn < turns - 1)
		{
			MapLocation current = open.poll();
			for (Direction d : RobotPlayer.customDirections)
			{
				...
				if (next.equals(end))
				{
					from[end.x][end.y] = current;
					return buildPath();
				}

				if (h[next.x][next.y] == 0)
					h[next.x][next.y] = heuristic(next.x, next.y, end.x, end.y); 
				if (map[next.x][next.y] == TerrainTile.VOID)
					continue;
				int speed = moveSpeed(next, d);
				int newg = g[current.x][current.y] + speed;
