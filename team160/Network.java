...
public class Network
{
	...
	private NetworkNode getAdjacentNode(MapLocation loc)
	{
		for (int i = -1; i <= 1; i++)
		{
			for (int j = -1; j <= 1; j++)
			{
				if ((i != 0 || j != 0) && i + loc.x >= 0 && i + loc.x < mapWidth && j + loc.y >= 0 && j + loc.y < mapHeight && nodeMap[i + loc.x][j + loc.y] != null)
	...
	private static Path buildPath(NetworkNode start, NetworkNode end, MapLocation adjStart, MapLocation adjEnd)
	{
	...
	public void printMap()
	{
		for (int i = 0; i < nodeMap[0].length; i++)
		{
			...
			{
				System.out.print(nodeMap[j][i] == null ? "." : nodeMap[j][i].links.size());
			}
			System.out.println();
		}
		System.out.println();
