...
{
	private static final int[] directionalLooks = new int[] { 0, 1, -1, 2, -2, 3, -3, 4 };
	private static final int[] directionalLooksReversed = new int[] { 0, -1, 1, -2, 2, -3, 3, 4 };
	private static final int[] attackLooks = new int[] { 1, -1, 0, 2, -2, 3, -3, 4 };
...
//		rc.setIndicatorString(2, "");
		...
		if (currentJob != null)
		{
			...
			if (currentJob != null && (attitude == 2 || myLoc.distanceSquaredTo(currentJob.location) < 10))
			{
				currentJob.claim(rc);
...
//			rc.setIndicatorString(0, "Goal " + goal);
			...
			if (attitude < 0 && tryToFlee()) // Flee if outnumbered and possible
				return;
			if (bestTarget != null && bestTargetDistance <= 10) // Attack if enemy in range
			{
				rc.attackSquare(bestTarget.location);
				Channels.stateMap.broadcast(rc, myLoc, State.ATTACKING);
			}
			else if (attitude > 0 && bestTarget != null && bestTargetDistance <= 21) // charge if agressive and ready
			{
				attackMove(bestTarget.location);
			}
//			else if (attitude == 0 && bestTargetDistance > 21 && alertLevel == AlertLevel.ORANGE)
//			{
...
//			}
			...
			{
				if (currentPath != null && currentPath.end.equals(goal))
					followCurrentPath(attitude == 0 ? bestTarget.location : null);
				else
				{
					if (currentPath != null)
						followCurrentPath(attitude == 0 ? bestTarget.location : null);
	...
	private static void updateAlertLevel() throws GameActionException
	{
		...
		if (myPASTR != null && myLoc.distanceSquaredTo(myPASTR) < 16  && alertLevel == AlertLevel.ORANGE)
			alertLevel = AlertLevel.GREEN;
	}

	private static MapLocation currentGoal() throws GameActionException
	{
		MapLocation goal = Channels.readLocation(rc, Channels.armyTargetLocation);
		...
		if (currentJob != null)
		{
			...
			case NT_OFFENSIVE:
				...
				{
					if (enemySoldierCount < rc.senseNearbyGameObjects(Robot.class, 36, RobotPlayer.team).length)
					{
			...
			case BUILD_PASTR:
				...
				if (myLoc.equals(currentJob.location))
				{
					if (enemySoldierCount < rc.senseNearbyGameObjects(Robot.class, 36, RobotPlayer.team).length)
					{
						...
						return null;
//						}
//						Robot[] allies = rc.senseNearbyGameObjects(Robot.class, myLoc, 2, rc.getTeam());
//						if (allies.length == 0)
//							lonelyTurns++;
//						for (Robot r : allies)
//						{
//							RobotInfo info = rc.senseRobotInfo(r);
//							if ((info.isConstructing && info.constructingType == RobotType.NOISETOWER && info.constructingRounds <= 50) || (info.type == RobotType.NOISETOWER))
//							{
//								rc.construct(RobotType.PASTR);
//								return null;
//							}
...
//						}
	...
	private static void followCurrentPath(MapLocation avoid) throws GameActionException
	{
		MapLocation nextPoint = currentPath.get(currentPathIndex);
		if (myLoc.isAdjacentTo(nextPoint) || myLoc.equals(nextPoint))
		{
			// clearTrail();
			currentPathIndex = Math.min(currentPath.size() - 1, currentPathIndex + 1);
	...
	private static void updateNearbyRobots() throws GameActionException
	{
		...
		if (alertLevel == AlertLevel.GREEN)
			enemyRobots = rc.senseNearbyGameObjects(Robot.class, 36, RobotPlayer.team.opponent());
		else
			enemyRobots = rc.senseNearbyGameObjects(Robot.class, 21, RobotPlayer.team.opponent());
		...
		for (Robot r : enemyRobots)
		{
			...
			case NOISETOWER:
				...
				break;
			case PASTR:
				if (bestTarget == null || bestTarget.type != RobotType.SOLDIER || bestTargetDistance > 10)
				{
					bestTarget = info;
					bestTargetDistance = distance;
					bestHealth = info.health;
				...
				break;
			case SOLDIER:
				if (info.isConstructing && info.constructingType == RobotType.PASTR) // TODO
					break;

				enemySoldierCount++;
				if (distance <= 10)
					attackableSoldierCount++;
				if (bestTarget == null || distance < bestTargetDistance || (distance <= 10 && bestTarget.type != RobotType.SOLDIER) || (distance == bestTargetDistance && info.health < bestHealth) || (distance == bestTargetDistance && info.health <= bestHealth && info.robot.getID() < bestID))
				{
					bestTargetDistance = distance;
					bestHealth = info.health;
					bestID = info.robot.getID();
		...
		if (bestTarget != null)
			alliedRobots = rc.senseNearbyGameObjects(Robot.class, bestTarget.location, 21, RobotPlayer.team);
		else
			alliedRobots = rc.senseNearbyGameObjects(Robot.class, 36, RobotPlayer.team);
		...
		for (Robot r : alliedRobots)
		{
			...
			case SOLDIER:
				...
				{
					int distance = bestTarget.location.distanceSquaredTo(info.location);
					if (distance <= 10)
					{
						attackingAllyCount++;
					}
					enemiesAroundClosestAlly = Math.max(enemiesAroundClosestAlly, Channels.radarMap.read(rc, info.location));
					//					if (closestAlly == null || distance < closestAllyDistance)
					//					{
		...
		//		if (closestAlly != null)
		//			enemiesAroundClosestAlly = Channels.radarMap.read(rc, closestAlly.location);
...
//		rc.setIndicatorString(0, "enemiesAroundClosestAlly: " + enemiesAroundClosestAlly);
...
//		}
		...
		case 0:
			if (enemySoldierCount == 0)
				return 2;
			if (enemySoldierCount <= nearbyAllyCount && (alertLevel != AlertLevel.GREEN || enemiesAroundClosestAlly <= nearbyAllyCount))
				return 1;
			return 0;
		case 1:
			if (attackingAllyCount == 0 && rc.getHealth() < bestTarget.health && alertLevel == AlertLevel.GREEN)
				return -1;
			return 0;
		default:
			if (attackableSoldierCount > attackingAllyCount + 1 && alertLevel == AlertLevel.GREEN)
				return -1;
			return 0;
		}
	}
	...
	private static boolean tryToFlee() throws GameActionException
	{
		...
		for (int look : bestTarget == null ? directionalLooks : directionalLooksReduced)
		{
			Direction trialDir = Direction.values()[(dirIndex + look) % 8];
			MapLocation nextLoc = myLoc.add(trialDir);
			if (canSafelyMove(trialDir, false, true) && (bestTarget == null || nextLoc.distanceSquaredTo(bestTarget.location) > 10))
			{
				rc.move(trialDir);
				Channels.stateMap.broadcast(rc, myLoc.add(trialDir), State.FLEEING);
				break;
			}
		}
	...
	private static boolean canAvoid(MapLocation goal, MapLocation avoid)
	{
		...
		if (toGoal.equals(toAvoid) || toGoal.equals(toAvoid.rotateLeft()) || toGoal.equals(toAvoid.rotateRight()))
			return false;
		return true;
	}


	private static void ninjaAvoid(MapLocation loc, MapLocation avoid) throws GameActionException
	{
//		rc.setIndicatorString(2, "Trying to avoid " + Clock.getRoundNum());
		...
		{
			Direction trialDir = Direction.values()[(dirIndex + look) % 8];
			if (forbiddenDir1.equals(trialDir) || forbiddenDir2.equals(trialDir) || forbiddenDir3.equals(trialDir))
				continue;
...
//				System.out.println("Using ninja avoid!");
	...
	private static void tryToAvoid(MapLocation loc, MapLocation avoid) throws GameActionException
	{
//		rc.setIndicatorString(2, "Trying to avoid " + Clock.getRoundNum());
		Direction bestDirection = myLoc.directionTo(loc);
		if (bestDirection == Direction.NONE || bestDirection == Direction.OMNI)
	...
	private static void attackMove(MapLocation loc) throws GameActionException
	{
		...
		if (bestDirection == Direction.NONE || bestDirection == Direction.OMNI)
			return;
		int dirIndex = bestDirection.ordinal() + 8;
		for (int look : bestDirection.isDiagonal() ? attackLooks : directionalLooks)
		{
			Direction trialDir = Direction.values()[(dirIndex + look) % 8];
			if (canSafelyMove(trialDir, true, true))
			{
	...
	private static void moveToward(MapLocation loc) throws GameActionException
	{
		...
		if (bestDirection == Direction.NONE || bestDirection == Direction.OMNI)
			return;
		int dirIndex = bestDirection.ordinal() + 8;
		int[] looks = ((rng.nextInt() + rc.getRobot().getID()) % 2) == 0 ? directionalLooks : directionalLooksReversed;
		for (int look : looks)
		{
			Direction trialDir = Direction.values()[(dirIndex + look) % 8];
			if (canSafelyMove(trialDir, true, true))
	...
	private static boolean canSafelyMove(Direction d, boolean avoidTrail, boolean avoidEnemyHQ)
	{
		if (!rc.canMove(d))
			return false;
		...
		if (avoidTrail)
		{
			...
			{
				if (tLoc != null && !tLoc.equals(myLoc) && (next.isAdjacentTo(tLoc) || next.equals(tLoc)))
					return false;
			}
		}
		if (avoidEnemyHQ)
		{
			MapLocation loc2 = next.add(next.directionTo(rc.senseEnemyHQLocation()));
