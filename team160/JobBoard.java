...
public class JobBoard
{
	...
	public void add(RobotController rc, JobType type, MapLocation loc) throws GameActionException
	{
		int count = jobCount(rc);
	...
	public Job getJobAt(RobotController rc, MapLocation loc) throws GameActionException
	{
		int count = jobCount(rc);
	...
	public Job getAvailableJob(RobotController rc) throws GameActionException
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			JobType type = JobType.values()[rc.readBroadcast(band + 1 + jobLength * i + 0)];
	...
	private void printBand(RobotController rc, int x) throws GameActionException
	{
		System.out.println();
		for (int i = 0; i < x; i++)
		{
