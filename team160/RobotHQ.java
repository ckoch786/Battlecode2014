...
//				System.out.println("PASTR turn: " + pastrBuildingTurn);
	...
	public static void run(RobotController rc) throws GameActionException
	{
		...
		{
			for (Robot r : rc.senseNearbyGameObjects(Robot.class, RobotType.HQ.sensorRadiusSquared, RobotPlayer.team.opponent()))
			{
...
//			System.out.println("PASTR Location computed! " +  myPASTRLocation);
			tryToSpawn();
			adjustPASTRLocation();
//			System.out.println("PASTR Location optimized! " +  myPASTRLocation);
			pathfindingQueue.add(new MapLocation[] { rallyPoint, myPASTRLocation });
			if (rallyPoint != myHQ)
				pathfindingQueue.add(new MapLocation[] { myHQ, myPASTRLocation });
		}
		...
		if (!pathfindingQueue.isEmpty())
		{
			...
			{
				Path path = AStarJPS.find(map, requestedPath[0], requestedPath[1], turnsLeft);
...
//						System.out.println("WARNING: Invalid path requested! " + requestedPath[0] + " " + requestedPath[1]);
						if (requestedPath[0].equals(myPASTRLocation) || requestedPath[1].equals(myPASTRLocation))
	...
	private static void strategy() throws GameActionException
	{
		...
		{
			if (lastStep < enemyMilk / GameConstants.OPPONENT_MILK_SENSE_ACCURACY)
			{
				lastStep = (int) (enemyMilk / GameConstants.OPPONENT_MILK_SENSE_ACCURACY);
				lastMilkRate = GameConstants.OPPONENT_MILK_SENSE_ACCURACY / turnsWithPASTR;
				turnsWithPASTR = 0;
			}
			turnsWithPASTR++;
		}
		...
		boolean enemyHasAPASTR = enemyPASTRs.length > 0 && enemyPASTRs[0].distanceSquaredTo(enemyHQ) > 25;
		boolean endIsNear = enemyMilk >= 5000000 || Clock.getRoundNum() > 1750;
		boolean enemyHasMoreMilk = estimatedEnemyMilk > myMilk;

		//		if (pastrUnderAttack || (enemyHasAPASTR && (enemyHasMoreMilk || enemyHasReached50)))
		//		{
		...
		if (enemyHasAPASTR && (myPASTR.length == 0 || (enemyMilk >= 2500000 && enemyHasMoreMilk || (enemyHasBuiltFirst && !destroyedPASTR))))
		{
			Channels.broadcastLocation(rc, Channels.armyTargetLocation, enemyPASTRs[0]);
			alert = AlertLevel.ORANGE;
//			if (endIsNear)
//			{
//				alert = AlertLevel.RED;
//			}
			...
			Channels.broadcastLocation(rc, Channels.armyTargetLocation, myPASTRLocation);
...
//		rc.setIndicatorString(0, "WOLOLO!   " + alert.ordinal());
		...
		if (map != null && !pastrRequested && (destroyedPASTR || Clock.getRoundNum() > pastrBuildingTurn || enemyMilk > GameConstants.WIN_QTY / 4))
		{
			pastrRequested = true;
			requestPASTRCreation();
		}
		if (myPASTRLocation != null && invalidPASTRLocation && myPASTR.length == 0) // TODO
		{
			// requestPASTRCreation();
...
//			System.out.println("New pastr requested");
		...
		if ((!pastrMoved && !pastrBuilt && Clock.getRoundNum() > 1000) || invalidPASTRLocation)
		{
			pastrMoved = true;
		...
		if (enemyPASTRs != null && enemyPASTRs.length > 0)
		{
			...
			if (target.distanceSquaredTo(enemyHQ) > 25)
			{
				...
				if (!targetsPathfinded.contains(target))
				{
					...
					pathfindingQueue.add(new MapLocation[] { rallyPoint, target });
					if (rallyPoint != myHQ)
						pathfindingQueue.add(new MapLocation[] { myHQ, target });
					pathfindingQueue.add(new MapLocation[] { myPASTRLocation, target });
				}
			}
		}
	}
	...
	private static void adjustPASTRLocation()
	{
		if (myPASTRLocation != null)
		{
			MapLocation[] locs = MapLocation.getAllMapLocationsWithinRadiusSq(myPASTRLocation, 9);
	...
	{
		return x >= 0 && x < w && y >= 0 && y < h && (map[x][y] != TerrainTile.VOID);
	}
...
//	}
	...
	private static MapLocation computePASTRLocation() throws GameActionException
	{
		double[][] growth = rc.senseCowGrowth();
		int w = rc.getMapWidth();
		int h = rc.getMapHeight();
...
//					rate += growth[i][j + 1];
	...
	private static MapLocation buildableLocationNextTo(MapLocation start)
	{
		...
		for (int i = 0; i < 8; i++)
		{
			...
			if (l.x >= 0 && l.x < w && l.y >= 0 && l.y < h && (map[l.x][l.y] != TerrainTile.VOID))
				return l;
		}
		for (MapLocation l : MapLocation.getAllMapLocationsWithinRadiusSq(myPASTRLocation, 10))
			if (l.x >= 0 && l.x < w && l.y >= 0 && l.y < h && (map[l.x][l.y] != TerrainTile.VOID))
				return l;

		for (MapLocation l : MapLocation.getAllMapLocationsWithinRadiusSq(myPASTRLocation, 20))
			if (l.x >= 0 && l.x < w && l.y >= 0 && l.y < h && (map[l.x][l.y] != TerrainTile.VOID))
				return l;
	...
	private static void initMap()
	{
		...
		{
			if (l.x >= 0 && l.x < w && l.y >= 0 && l.y < h && (l.distanceSquaredTo(enemyHQ) != 25 || l.directionTo(enemyHQ).isDiagonal()))
			{
	...
	{
		return 5 * rc.getMapHeight() * rc.getMapWidth() * 10 / GameConstants.BYTECODE_LIMIT;
	}
}
