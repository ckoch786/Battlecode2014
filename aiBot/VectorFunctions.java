...
public class VectorFunctions {
	public static MapLocation findClosest(MapLocation[] manyLocs,
			MapLocation point) {
	...
	public static int findClosest(ArrayList<MapLocation> manyLocs,
			MapLocation point) {
		int closestDist = 10000000;
		int challengerDist = closestDist;
		int closestLoc = 0;
	...
	public static MapLocation mlsum(MapLocation m1, MapLocation m2) {
	...
	public static MapLocation bigBoxCenter(MapLocation bigBoxLoc, int bigBoxSize) {
		return mlsum(mlmultiply(bigBoxLoc, bigBoxSize), new MapLocation(
				bigBoxSize / 2, bigBoxSize / 2));
	}

	public static MapLocation[] robotsToLocations(Robot[] robotList,
			RobotController rc, boolean ignoreHQ) throws GameActionException {
		if (robotList.length == 0)
			return new MapLocation[] {};
		ArrayList<MapLocation> robotLocs = new ArrayList<MapLocation>();
		for (int i = 0; i < robotList.length; i++) {
			Robot r = robotList[i];
			RobotInfo anEnemyInfo = rc.senseRobotInfo(r);
	...
	public static MapLocation meanLocation(MapLocation[] manyLocs) {
		...
		for (MapLocation m : manyLocs) {
			runningTotal = mlsum(runningTotal, m);
		}
